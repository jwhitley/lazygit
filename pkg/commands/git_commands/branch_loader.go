...
func (self *BranchLoader) obtainBranches() []*models.Branch {
	...
	return lo.FilterMap(outputLines, func(line string, _ int) (*models.Branch, bool) {
		...
		storeCommitterDateAsRecency := self.AppState.LocalBranchSortOrder != "recency"
		return obtainBranch(split, storeCommitterDateAsRecency), true
	})
}
...
func obtainBranch(split []string, storeCommitterDateAsRecency bool) *models.Branch {
	headMarker := split[0]
	fullName := split[1]
	upstreamName := split[2]
	track := split[3]
	subject := split[4]
	unixTimestamp := split[5]
	commitHash := split[6]
	...
	recency := ""
	if storeCommitterDateAsRecency {
		if unixTimestampInt, err := strconv.ParseInt(unixTimestamp, 10, 64); err == nil {
			recency = utils.UnixToTimeAgo(unixTimestampInt)
		}
	}
